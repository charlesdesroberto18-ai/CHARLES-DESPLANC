...
export const BottomNav: React.FC<Props> = ({ currentView, onChangeView }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 lg:left-auto lg:right-auto lg:top-0 lg:bottom-auto lg:w-auto lg:border-r lg:border-t-0 lg:border-b-0 lg:h-screen bg-white border-t border-gray-200 pb-safe pt-2 px-2 lg:pt-0 lg:px-0 lg:pb-0 lg:pl-2 shadow-[0_-4px_10px_-4px_rgba(0,0,0,0.05)] lg:shadow-[4px_0_10px_-4px_rgba(0,0,0,0.05)] z-40">
      <div className="flex justify-around items-end pb-2 lg:flex-col lg:items-center lg:justify-start lg:gap-0 overflow-x-auto lg:overflow-y-auto lg:h-full">
        {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center min-w-[50px] lg:min-w-0 lg:w-full py-1 lg:py-3 transition-colors duration-200 ${
              ...
              >
                ...
                <span className={`text-[10px] lg:text-xs font-medium ${isActive ? 'font-bold' : ''}`}>{item.label}</span>
